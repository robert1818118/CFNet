...
if __name__ == '__main__':
    ...
    for i in range(5):
        with h5py.File('./input/mmi_sample_label_{}.h5'.format(i+1), 'r') as f:
            x_l_tr_mmi = np.array(f['x_l_tr_mmi'])
            x_g_tr_mmi = np.array(f['x_g_tr_mmi'])
            x_c_tr_mmi = np.array(f['x_c_tr_mmi'])
            x_l_te_mmi = np.array(f['x_l_te_mmi'])
            x_g_te_mmi = np.array(f['x_g_te_mmi'])
            x_c_te_mmi = np.array(f['x_c_te_mmi'])
        ...
        print(x_l_tr_mmi.shape, x_g_tr_mmi.shape, x_c_tr_mmi.shape, x_l_te_mmi.shape, x_g_te_mmi.shape, x_c_te_mmi.shape, label_tr.shape, label_te.shape)

        file_path = './models/MMI_best_model_{}.h5'.format(i + 1)
        ...
        y_pred = model.predict([x_l_te_mmi, x_g_te_mmi, x_c_te_mmi], batch_size=32, verbose=1)
        y_pred = np.argmax(y_pred, axis=1)
        y_true = np.argmax(label_te, axis=1)

        ACC.append(balanced_accuracy_score(y_true=y_true, y_pred=y_pred))
        print(ACC)
